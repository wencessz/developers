...
Class CashMachine{
  ...
  Public function getMoney($money=null){
	...
	while($valuesleft > 0){ //while the money still left
	  $prevalue = $valuesleft;
	  foreach($this->availableMoney as $key => $value){ //loop around the money in the machine
		if($this->checkIsValid($valuesleft,$value)){ //if valueleft - value is still valid
		  $valuesleft =  $valuesleft - $value;
	      $return[] = $value;
		  break;
		}
	  }
	  if($valuesleft == $prevalue){ //if we checked all values and anything has changed, its mean that is not a valid number
